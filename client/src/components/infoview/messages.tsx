...
function Error({error, uiMode} : {error : InteractiveDiagnostic, uiMode : UIMode}) {
  ...
  return <div className={severityClass + ' ml1 message'}>
    {uiMode !== 'typewriter' && <p className="mv2">{title}</p>}
...
/** A list of messages (info/warning/error) that are produced after this command */
export function Errors ({errors, uiMode} : {errors : InteractiveDiagnostic[], uiMode : UIMode}) {
  return <div>
    {errors.map((err, i) => (<Error key={`error-${i}`} error={err} uiMode={uiMode}/>))}
  </div>
}
...
const MessageView = React.memo(({uri, diag}: MessageViewProps) => {
    ...
    // <details open>
        ...
        <div className={severityClass + ' ml1 message'}>
            {!(uiMode === 'typewriter' && !lockUIMode) && <p className="mv2">{title}</p>}
            <pre className="font-code pre-wrap">
                <InteractiveMessage fmt={message} />
            </pre>
        </div>
