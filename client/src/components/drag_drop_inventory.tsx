...
export function DragDropInventory({levelInfo, openDoc, lemmaTab, setLemmaTab, enableAll=false} :
  {
...
export function DragDropInventoryPanel({levelInfo, visible = true}) {
  const gameId = React.useContext(GameIdContext)
  ...
  return <div className={`column inventory-panel ${visible ? '' : 'hidden'}`}>
    ...
    {/* {inventoryDoc ?
      ...
      :
      <DragDropInventory levelInfo={levelInfo} openDoc={setInventoryDoc} enableAll={true} lemmaTab={lemmaTab} setLemmaTab={setLemmaTab}/>
    } */}
  </div>
}
